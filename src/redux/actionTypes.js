// Session
export const JOIN_IP_CHANGED = "join_ip_changed";
export const SINGER_NAME_CHANGED = "singer_name_changed";
export const SINGER_COLOR_CHANGED = "singer_color_changed";

export const JOIN_SESSION = "join_session";
export const DISCONNECTED = "disconnected";

export const QUEUE_UPDATED = "queue_updated";

export const SHOW_MODAL = "show_modal";
export const HIDE_MODAL = "hide_modal";

// Search
export const SEARCH = "search";
export const SEARCH_TERM_CHANGED = "search_term_changed";

export const SONG_SELECTED = "song_selected";

// Player
export const PLAYER_STATE_UPDATED = "player_state_updated";

